...
class Classifier():
	...
	def findCenterAverage(self, frame_count, eye_center, eye_cnt):
		if 100 < frame_count < 180:
			self.increaseAccumulators(eye_center, eye_cnt)
	...
	def classify(self, frame_count, eye_center, eye_cnt):
		if frame_count > 180 and frame_count % 5 == 0: 
			...
			if xDiff > yDiff:
				...
				if cntDiff >= 100:
					if self.gazeAverage[1] < self.centerAverage[1] and yDiff > 3:
						self.direction = 'up'
					else:
						self.direction = 'down'
				else:
